...
public class ClassService {
    ...
    private Specification<ClassEntity> buildSpec(ClassListRequest req) {
        return (root, query, cb) -> {
            ...
            // Join Course
            Join<ClassEntity, Course> courseJoin = root.join("course", JoinType.LEFT);
            // Nếu ClassEntity dùng courseId (Long) thay vì @ManyToOne thì dùng subquery
            // → xem ghi chú bên dưới

            // Tìm theo mã lớp hoặc tên môn học
            if (req.getKeyword() != null && !req.getKeyword().isBlank()) {
                String pattern = "%" + req.getKeyword().trim().toLowerCase() + "%";
                predicates.add(cb.or(
                        cb.like(cb.lower(root.get("code")), pattern),
                        cb.like(cb.lower(courseJoin.get("name")), pattern)
                ));
            }
            ...
            // Lọc theo khoa (qua Course → Department)
            if (req.getDepartmentId() != null) {
                predicates.add(cb.equal(courseJoin.get("department").get("id"), req.getDepartmentId()));
            }
